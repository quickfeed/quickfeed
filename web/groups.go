...
func (s *QuickFeedService) getGroup(request *types.GetGroupRequest) (*types.Group, error) {
...
func (s *QuickFeedService) getGroups(request *types.CourseRequest) (*types.Groups, error) {
	groups, err := s.db.GetGroupsByCourse(request.GetCourseID())
	if err != nil {
		return nil, err
	}
	return &types.Groups{Groups: groups}, nil
}

// getGroupByUserAndCourse returns the group of the given user and course.
func (s *QuickFeedService) getGroupByUserAndCourse(request *types.GroupRequest) (*types.Group, error) {
...
func (s *QuickFeedService) deleteGroup(ctx context.Context, sc scm.SCM, request *types.GroupRequest) error {
	...
	repo, err := s.getRepo(course, group.GetID(), types.Repository_GROUP)
...
func (s *QuickFeedService) createGroup(request *types.Group) (*types.Group, error) {
...
func (s *QuickFeedService) updateGroup(ctx context.Context, sc scm.SCM, request *types.Group) error {
	course, group, err := s.getCourseGroup(&types.GroupRequest{
	...
	if group.Name != request.Name && group.Status == types.Group_PENDING {
	...
	newGroup.Status = types.Group_APPROVED
...
func (s *QuickFeedService) getGroupUsers(request *types.Group) ([]*types.User, error) {
	...
	for _, user := range request.Users {
		...
		case enrollment.GroupID > 0 && enrollment.GroupID != request.ID:
			...
			return nil, status.Errorf(codes.InvalidArgument, "user already enrolled in another group")
		case enrollment.Status < types.Enrollment_STUDENT:
...
func (s *QuickFeedService) getCourseGroup(request *types.GroupRequest) (*types.Course, *types.Group, error) {
	group, err := s.db.GetGroup(request.GetGroupID())
	if err != nil {
		return nil, nil, err
	}

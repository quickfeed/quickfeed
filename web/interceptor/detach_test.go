...
type cancellationProbe struct {
	...
	hasDeadline chan bool     // reports whether the interceptor added a deadline
}
...
// RebuildSubmissions manages its own deadlines but must be shielded from the client's cancellation.
func (c *cancellationProbe) RebuildSubmissions(ctx context.Context, _ *qf.RebuildRequest) (*qf.Void, error) {
	c.observe(ctx)
	return &qf.Void{}, nil
}
...
func TestDetachInterceptor(t *testing.T) {
	tests := []struct {
		...
		{
			name: "rebuild submissions survives client disconnect",
			call: func(ctx context.Context, client qfconnect.QuickFeedServiceClient) error {
				_, err := client.RebuildSubmissions(ctx, &qf.RebuildRequest{})
				return err
			},
			wantCanceled: false,
			wantDeadline: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if hasDeadline := <-probe.hasDeadline; hasDeadline != tt.wantDeadline {
				t.Errorf("handler context has deadline = %t, want %t", hasDeadline, tt.wantDeadline)
			}
			<-done
		})
	}

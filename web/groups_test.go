...
func TestNewGroup(t *testing.T) {
	...
	if err := db.CreateEnrollment(&types.Enrollment{UserID: user.ID, CourseID: course.ID}); err != nil {
		t.Fatal(err)
	}
	...
	createGroupRequest := &types.Group{Name: "Heins-Group", CourseID: course.ID, Users: []*types.User{{ID: user.ID}}}
	...
	gotGroup, err := ags.GetGroup(ctx, &types.GetGroupRequest{GroupID: wantGroup.ID})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCreateGroupWithMissingFields(t *testing.T) {
	...
	users := []*types.User{{ID: user.ID}}
	group_wo_course_id := &types.Group{Name: "Hein's Group", Users: users}
	group_wo_name := &types.Group{CourseID: course.ID, Users: users}
	group_wo_users := &types.Group{Name: "Hein's Group", CourseID: course.ID}
...
func TestNewGroupTeacherCreator(t *testing.T) {
	...
	if err := db.CreateEnrollment(&types.Enrollment{UserID: teacher.ID, CourseID: course.ID}); err != nil {
		t.Fatal(err)
	}
	if err := db.UpdateEnrollment(&types.Enrollment{
		UserID:   teacher.ID,
	...
	users := []*types.User{{ID: user.ID}}
	createGroupRequest := &types.Group{Name: "HeinsGroup", CourseID: course.ID, Users: users}
	...
	_, err = ags.GetGroup(ctx, &types.GetGroupRequest{GroupID: wantGroup.ID})
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewGroupStudentCreateGroupWithTeacher(t *testing.T) {
	...
	group_req := &types.Group{Name: "HeinsGroup", CourseID: course.ID, Users: []*types.User{{ID: user.ID}, {ID: teacher.ID}}}
...
func TestStudentCreateNewGroupTeacherUpdateGroup(t *testing.T) {
	...
	admin := qtest.CreateFakeUser(t, db, 1)
	course := types.Course{Provider: "fake", OrganizationID: 1}
	if err := db.CreateCourse(admin.ID, &course); err != nil {
		t.Fatal(err)
	}
	...
	if err := db.CreateEnrollment(&types.Enrollment{UserID: user1.ID, CourseID: course.ID}); err != nil {
		t.Fatal(err)
	}
	if err := db.UpdateEnrollment(&types.Enrollment{
		UserID:   user1.ID,
	...
	if err := db.CreateEnrollment(&types.Enrollment{UserID: user2.ID, CourseID: course.ID}); err != nil {
		t.Fatal(err)
	}
	if err := db.UpdateEnrollment(&types.Enrollment{
		UserID:   user2.ID,
	...
	if err := db.CreateEnrollment(&types.Enrollment{UserID: user3.ID, CourseID: course.ID}); err != nil {
		t.Fatal(err)
	}
	if err := db.UpdateEnrollment(&types.Enrollment{
		UserID:   user3.ID,
	...
	createGroupRequest := &types.Group{Name: "HeinsTwoMemberGroup", CourseID: course.ID, Users: []*types.User{user1, user2}}
	...
	updateGroupRequest := &types.Group{ID: gotGroup.ID, Name: "Heins3MemberGroup", CourseID: course.ID, Users: []*types.User{user1, user2, user3}}
	...
	wantGroup.Status = types.Group_APPROVED
	...
	updateGroupRequest1 := &types.Group{ID: gotGroup.ID, Name: "Hein's single member Group", CourseID: course.ID, Users: []*types.User{user1}}
...
func TestDeleteGroup(t *testing.T) {
	...
	ctx = qtest.WithUserContext(context.Background(), teacher)
	if _, err := ags.CreateEnrollment(ctx, &types.Enrollment{UserID: teacher.ID, CourseID: testCourse.ID}); err != nil {
		t.Fatal(err)
	}

	// update enrollment from pending->student->teacher; must be done by admin
	ctx = qtest.WithUserContext(context.Background(), admin)
	if _, err := ags.UpdateEnrollments(ctx, &types.Enrollments{
	...
	user := qtest.CreateFakeUser(t, db, 2)
	ctx = qtest.WithUserContext(context.Background(), user)
	if _, err := ags.CreateEnrollment(ctx, &types.Enrollment{UserID: user.ID, CourseID: testCourse.ID}); err != nil {
		t.Fatal(err)
	}
	...
	group := &types.Group{Name: "TestDeleteGroup", CourseID: testCourse.ID, Users: []*types.User{user}}
	ctx = qtest.WithUserContext(context.Background(), user)
	...
	ctx = qtest.WithUserContext(context.Background(), teacher)
	_, err = ags.DeleteGroup(ctx, &types.GroupRequest{GroupID: respGroup.ID, CourseID: testCourse.ID})
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestGetGroup(t *testing.T) {
	...
	user := qtest.CreateFakeUser(t, db, 2)
	if err := db.CreateEnrollment(&types.Enrollment{UserID: user.ID, CourseID: testCourse.ID}); err != nil {
		t.Fatal(err)
	}
	if err := db.UpdateEnrollment(&types.Enrollment{
		UserID:   user.ID,
	...
	group := &types.Group{Name: "TestGroup", CourseID: testCourse.ID, Users: []*types.User{user}}
...
func TestPatchGroupStatus(t *testing.T) {
	...
	if err := db.UpdateUser(&types.User{ID: teacher.ID, IsAdmin: true}); err != nil {
		t.Fatal(err)
	}
...
func TestGetGroupByUserAndCourse(t *testing.T) {
	...
	wantGroup, err := ags.GetGroupByUserAndCourse(ctx, &types.GroupRequest{UserID: user1.ID, CourseID: course.ID})
	if err != nil {
		t.Error(err)
	}
	gotGroup, err := ags.GetGroup(ctx, &types.GetGroupRequest{GroupID: group.ID})
	if err != nil {
		t.Fatal(err)
	}
	if diff := cmp.Diff(wantGroup, gotGroup, protocmp.Transform()); diff != "" {
...
func TestDeleteApprovedGroup(t *testing.T) {
	...
	createdGroup.Status = types.Group_APPROVED
	...
	// delete the group
	if _, err = ags.DeleteGroup(ctx, &types.GroupRequest{CourseID: course.ID, GroupID: createdGroup.ID}); err != nil {
		t.Fatal(err)
	}
...
func TestGetGroups(t *testing.T) {
	...
	group1, err := ags.CreateGroup(ctx, &types.Group{Name: "Group1", CourseID: course.ID, Users: []*types.User{users[1], users[2]}})
	if err != nil {
		t.Fatal(err)
	}
	ctx = qtest.WithUserContext(context.Background(), users[5])
	group2, err := ags.CreateGroup(ctx, &types.Group{Name: "Group2", CourseID: course.ID, Users: []*types.User{users[4], users[5]}})
	if err != nil {
		t.Fatal(err)
	}
	wantGroups := &types.Groups{Groups: []*types.Group{group1, group2}}
	for _, grp := range wantGroups.Groups {
		for _, grpEnrol := range grp.Enrollments {
			grpEnrol.UsedSlipDays = []*types.UsedSlipDays{}
		}
	}

	// check that request on non-existent course returns error
	_, err = ags.GetGroupsByCourse(ctx, &types.CourseRequest{CourseID: 15})
	if err == nil {
	...
	ctx = qtest.WithUserContext(context.Background(), admin)
	gotGroups, err := ags.GetGroupsByCourse(ctx, &types.CourseRequest{CourseID: course.ID})
	if err != nil {
		t.Fatal(err)
	}

...
func (s *QuickFeedService) getCurrentUser(ctx context.Context) (*types.User, error) {
...
func (s *QuickFeedService) getSCM(ctx context.Context, user *types.User, provider string) (scm.SCM, error) {
	providers, err := s.GetProviders(ctx, &types.Void{})
	if err != nil {
		return nil, err
	}
...
func (s *QuickFeedService) hasCourseAccess(userID, courseID uint64, check func(*types.Enrollment) bool) bool {
...
func (s *QuickFeedService) isEnrolled(userID, courseID uint64) bool {
	return s.hasCourseAccess(userID, courseID, func(e *types.Enrollment) bool {
		return e.Status == types.Enrollment_STUDENT || e.Status == types.Enrollment_TEACHER
	})
}
...
func (s *QuickFeedService) isValidSubmissionRequest(submission *types.SubmissionRequest) bool {
	...
	return s.hasCourseAccess(submission.GetUserID(), submission.GetCourseID(), func(e *types.Enrollment) bool {
		return e.Status >= types.Enrollment_STUDENT
	})
}
...
func (s *QuickFeedService) isValidSubmission(submissionID uint64) bool {
	...
	assignment, err := s.db.GetAssignment(&types.Assignment{ID: submission.GetAssignmentID()})
	if err != nil {
		return false
	}

	request := &types.SubmissionRequest{
...
func (s *QuickFeedService) isTeacher(userID, courseID uint64) bool {
	return s.hasCourseAccess(userID, courseID, func(e *types.Enrollment) bool {
		return e.Status == types.Enrollment_TEACHER
	})
}
...
func (s *QuickFeedService) getUserAndSCM(ctx context.Context, provider string) (*types.User, scm.SCM, error) {
...
// All errors are logged, but only a single error is returned to the client.
// This is a helper method to facilitate consistent treatment of errors and logging.
func (s *QuickFeedService) getUserAndSCMForCourse(ctx context.Context, courseID uint64) (*types.User, scm.SCM, error) {

...
func (s *QuickFeedService) removeFromGroup(ctx context.Context, sc scm.SCM, enrolled *qf.Enrollment) error {
	...
	if err := updateGroupMembers(ctx, sc, group, course.GetScmOrganizationName()); err != nil {
		if !errors.Is(err, scm.ErrNotFound) {
			return fmt.Errorf("updating %s repository for group %q: %w", course.GetCode(), group.GetName(), err)
		}
		// group repository already removed on GitHub, e.g., by a previously interrupted
		// reject; remove the stale repository record so database cleanup can continue.
		logger.Debug("group repository not found on SCM", label.Group, group.GetName(), label.Error, err)
		if err := s.db.DeleteRepository(repo.GetScmRepositoryID()); err != nil {
			logger.Debug("failed to delete group repository from database", label.Error, err)
			// continue; the enrollment removal must not be blocked by this
		}
	}
	return nil
}

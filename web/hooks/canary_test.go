...
func setupCanaryCourse(t *testing.T, db database.Database, runScript string) *qf.Course {
	...
	scripts := filepath.Join(root, canaryTestOrg, qf.TestsRepo, ci.ScriptsDir)
	mkdir(t, scripts)
...
// snapshotRunner is a ci.Runner that changes the live course repositories while
// the run is in progress, and checks that the run sees neither that change nor
// the live repositories themselves.
type snapshotRunner struct {
	t              *testing.T
	liveCourseDir  string
	snapshotDir    string
	runWasObserved bool
}

// Run reports failures with t.Error rather than t.Fatal, since a runner is not
// guaranteed to be called on the test's own goroutine.
func (r *snapshotRunner) Run(_ context.Context, job *ci.Job) (string, error) {
	r.t.Helper()
	r.runWasObserved = true
	liveMarker := filepath.Join(r.liveCourseDir, qf.AssignmentsRepo, "lab1", "version.txt")
	if err := os.WriteFile(liveMarker, []byte("changed during run"), 0o600); err != nil {
		r.t.Error(err)
		return "", err
	}

	for source, target := range job.ReadOnlyMounts {
		if strings.HasPrefix(source, r.liveCourseDir+string(filepath.Separator)) {
			r.t.Errorf("RunTests() mounted live course repository %q at %q", source, target)
		}
		if filepath.Base(target) == qf.AssignmentsRepo {
			r.snapshotDir = filepath.Dir(source)
		}
		got, err := os.ReadFile(filepath.Join(source, "lab1", "version.txt"))
		if err != nil {
			if os.IsNotExist(err) {
				continue // only the assignments repository has the marker file
			}
			r.t.Error(err)
			return "", err
		}
		if string(got) != "before run" {
			r.t.Errorf("snapshot content of %q = %q, want %q", target, got, "before run")
		}
	}

	submission := filepath.Join(job.BindDir, qf.StudentRepoName("skeleton"), "lab1", "version.txt")
	got, err := os.ReadFile(submission)
	if err != nil {
		r.t.Error(err)
		return "", err
	}
	if string(got) != "before run" {
		r.t.Errorf("copied skeleton content = %q, want %q", got, "before run")
	}
	return "", nil
}

func TestRunCanaryUsesRepositorySnapshot(t *testing.T) {
	runner := &snapshotRunner{t: t}
	wh, course, ctx, _ := setupCanary(t, runner, canaryNoOutputScript)
	runner.liveCourseDir = course.CloneDir()
	marker := filepath.Join(course.CloneDir(), qf.AssignmentsRepo, "lab1", "version.txt")
	if err := os.WriteFile(marker, []byte("before run"), 0o600); err != nil {
		t.Fatal(err)
	}

	wh.runCanary(ctx, nil, course, qf.TestsRepo, canaryPush("lab1/lab1.go"))
	if !runner.runWasObserved {
		t.Fatal("runCanary() did not run the canary")
	}
	if runner.snapshotDir == "" {
		t.Fatal("RunTests() did not mount the assignments snapshot")
	}
	if _, err := os.Stat(runner.snapshotDir); !os.IsNotExist(err) {
		t.Errorf("snapshot directory still exists after run: %v", err)
	}
}

func TestRunCanarySkipsUnavailableAssignmentsClone(t *testing.T) {
	const skipMsg = "skipping test environment canary: assignments clone was not refreshed"
	wh, course, ctx, output := setupCanary(t, &ci.Local{}, canaryNoOutputScript)

	wh.runCanaryAfterUpdate(ctx, nil, course, qf.TestsRepo, canaryPush("lab1/lab1.go"), assignments.UpdateResult{})
	records := recordsWith(canaryRecords(t, output.String()), skipMsg)
	if len(records) != 1 {
		t.Fatalf("runCanaryAfterUpdate() logged %d %q records, want 1:\n%s", len(records), skipMsg, output.String())
	}
	if records[0].Level != slog.LevelWarn.String() {
		t.Errorf("runCanaryAfterUpdate() logged %q at level %s, want %s", skipMsg, records[0].Level, slog.LevelWarn.String())
	}
	assertNoCanaryRun(t, output.String())
}

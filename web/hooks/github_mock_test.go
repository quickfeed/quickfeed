...
func NewMockWebHook(logger *slog.Logger, secret string) *MockWebHook {
	return &MockWebHook{
		logger: logger,
		secret: secret,
...
func (wh *MockWebHook) Handle() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case *github.PushEvent:
			...
			wh.logger.Debug("received push event", label.Commit, commitID)
			if wh.dup.Duplicate(commitID) {
				wh.logger.Debug("ignoring duplicate push event", label.Commit, commitID)
				return
			}
...
func (wh *MockWebHook) handlePush(payload *github.PushEvent) {
	...
	wh.logger.Debug("received push event", label.Repository, payload.GetRepo().GetName(), "count", curCnt)
}

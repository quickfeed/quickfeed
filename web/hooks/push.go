...
func (wh GitHubWebHook) handlePush(ctx context.Context, payload *github.PushEvent) {
	...
	case repo.IsTestsRepo():
		...
		if update, err := assignments.UpdateFromCourseRepositories(ctx, wh.runner, wh.db, scmClient, course); err != nil {
			logger.Error("failed to update from course repositories", label.Error, err)
		} else {
			wh.runCanaryAfterUpdate(ctx, scmClient, course, repo.Name(), payload, update)
		}
	...
	case repo.IsAssignmentsRepo():
		...
		update, updateErr := assignments.UpdateFromCourseRepositories(ctx, wh.runner, wh.db, scmClient, course)
		if updateErr != nil {

...
func TestApproveEnrollmentDeletedSCMUser(t *testing.T) {
	...
	course := qtest.MockCourses[0]
	qtest.CreateCourse(t, db, admin, course)
	// Enrollment starts out PENDING so that PENDING -> STUDENT would otherwise be
	// a valid transition; this ensures the test verifies the deleted-user guard,
	// rather than passing because the requested transition is invalid regardless.
	qtest.EnrollUser(t, db, student, course, qf.Enrollment_PENDING)

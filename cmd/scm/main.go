...
func deleteRepositories(client *scm.SCM) cli.ActionFunc {
	...
	return func(c *cli.Context) error {
		...
		if c.Bool("all") {
			...
			repos, err := (*client).GetRepositories(ctx, &types.Organization{Path: c.String("namespace")})
			if err != nil {
				return err
			}
...
func deleteTeams(client *scm.SCM) cli.ActionFunc {
	...
	return func(c *cli.Context) error {
		...
		if c.Bool("all") {
			...
			teams, err := (*client).GetTeams(ctx, &types.Organization{Path: c.String("namespace")})
			if err != nil {
				return err
			}

...
func TestBunDBNonExistingTasksForAssignment(t *testing.T) {
	...
	if _, gotError := db.GetTasks(&qf.Task{AssignmentID: assignments[0].GetID()}); !errors.Is(gotError, sql.ErrNoRows) {
		t.Errorf("got error '%v' wanted '%v'", gotError, sql.ErrNoRows)
	}
}

// TestBunDBSynchronizeAssignmentTasks tests whether SynchronizeAssignmentTasks
// correctly synchronizes tasks in the database, and whether it returns the correct created and updated tasks.
func TestBunDBSynchronizeAssignmentTasks(t *testing.T) {

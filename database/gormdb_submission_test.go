...
func TestGormDBGetSubmissionForUser(t *testing.T) {
	...
	defer cleanup()
	query := &types.Submission{AssignmentID: 10, UserID: 10}
	if _, err := db.GetSubmission(query); err != gorm.ErrRecordNotFound {
		t.Errorf("have error '%v' wanted '%v'", err, gorm.ErrRecordNotFound)
	}
}

func setupCourseAssignment(t *testing.T, db database.Database) (*types.User, *types.Course, *types.Assignment) {
...
func TestGormDBUpdateSubmission(t *testing.T) {
	...
	if submissions[0].GetStatus() != types.Submission_NONE {
	...
	submissions[0].Status = types.Submission_APPROVED
	err = db.UpdateSubmission(submissions[0])
	if err != nil {
		t.Fatal(err)
	}
	...
	if submissions[0].GetStatus() != types.Submission_APPROVED {
...
func TestGormDBGetNonExistingSubmissions(t *testing.T) {
	db, cleanup := qtest.TestDB(t)
	defer cleanup()
	if _, err := db.GetLastSubmissions(10, &types.Submission{UserID: 10}); err != gorm.ErrRecordNotFound {
		t.Errorf("have error '%v' wanted '%v'", err, gorm.ErrRecordNotFound)
	}
}
...
func TestGormDBInsertBadSubmissions(t *testing.T) {
	...
	// expected to fail
	if err := db.CreateSubmission(&types.Submission{}); !errors.Is(err, database.ErrInvalidAssignmentID) {
		t.Fatal(err)
	}
	// expected to fail
	if err := db.CreateSubmission(&types.Submission{AssignmentID: 1}); !errors.Is(err, database.ErrInvalidSubmission) {
		t.Fatal(err)
	}
	// expected to fail
	if err := db.CreateSubmission(&types.Submission{UserID: 1}); !errors.Is(err, database.ErrInvalidAssignmentID) {
		t.Fatal(err)
	}
	// expected to fail with record not found
	if err := db.CreateSubmission(&types.Submission{AssignmentID: 1, UserID: 1}); !errors.Is(err, gorm.ErrRecordNotFound) {
		t.Fatal(err)
	}
	// expected to fail with record not found
	if err := db.CreateSubmission(&types.Submission{AssignmentID: 1, GroupID: 6}); !errors.Is(err, gorm.ErrRecordNotFound) {
		t.Fatal(err)
	}
	...
	// create a submission for the assignment for non-existing user; should fail
	if err := db.CreateSubmission(&types.Submission{AssignmentID: assignment.ID, UserID: 3}); !errors.Is(err, gorm.ErrRecordNotFound) {
		t.Fatal(err)
	}
	// create a submission for the assignment for non-existing user; should fail
	if err := db.CreateSubmission(&types.Submission{AssignmentID: assignment.ID, GroupID: 9}); !errors.Is(err, gorm.ErrRecordNotFound) {
		t.Fatal(err)
	}

	// create another submission for the assignment; now it should succeed
	if err := db.CreateSubmission(&types.Submission{AssignmentID: assignment.ID, UserID: user.ID}); err != nil {
		t.Fatal(err)
	}
}
...
func TestGormDBGetInsertSubmissions(t *testing.T) {
	...
	admin := qtest.CreateFakeUser(t, db, 10)
	c1 := &types.Course{OrganizationID: 1, Year: 1}
	c2 := &types.Course{OrganizationID: 2, Year: 2}
	...
	submissions, err := db.GetLastSubmissions(c1.ID, &types.Submission{UserID: user.ID})
	if err != nil {
		t.Fatal(err)
	}
	...
	data, err := db.GetLastSubmissions(c1.ID, &types.Submission{UserID: user.ID})
	if err != nil {
		t.Fatal(err)
	} else if len(data) != 2 {
	...
	data, err = db.GetLastSubmissions(c2.ID, &types.Submission{UserID: user.ID})
	if err != nil {
		t.Fatal(err)
	} else if len(data) != 0 {

...
func TestGormDBGetEmptyRepo(t *testing.T) {
	...
	defer cleanup()
	repos, err := db.GetRepository(&pb.Repository{RepositoryID: 10})
	if err != nil && err != gorm.ErrRecordNotFound {
		t.Fatalf("Expected no repositories, but got: %v: %v", repos, err)
	}
}
...
func TestGormDBGetSingleRepoWithUser(t *testing.T) {
	...
	if _, err := db.GetRepository(&pb.Repository{RepositoryID: repo.RepositoryID}); err != nil {
		t.Fatal(err)
	}
}
...
func TestGormDBGetCourseRepoType(t *testing.T) {
	...
	gotRepo, err := db.GetRepository(&pb.Repository{RepositoryID: repo.RepositoryID})
	if err != nil {
		t.Fatal(err)
	}
...
func TestGormDeleteRepo(t *testing.T) {
	...
	gotRepo, err := db.GetRepository(&pb.Repository{RepositoryID: repo.RepositoryID})
	if err != nil && err != gorm.ErrRecordNotFound {
		t.Fatalf("Expected no repositories, but got: %v: %v", gotRepo, err)
	}
}
...
func TestGetRepositoriesByOrganization(t *testing.T) {
	...
	gotRepo, err := db.GetRepository(&pb.Repository{OrganizationID: 120})
	if err == nil {
		t.Errorf("Expected 'ambiguous query' error, but got: %v", gotRepo)
	}
}

func TestGetRepositoryByCourseUserAndType(t *testing.T) {
	db, cleanup := qtest.TestDB(t)
	defer cleanup()
	...
	wantRepo := &repoUserTwo
	repoQuery := &pb.Repository{
		OrganizationID: course.OrganizationID,
		UserID:         userTwo.ID,
		RepoType:       pb.Repository_USER,
	}
	gotRepo, err := db.GetRepository(repoQuery)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetRepositoryByCourseAndType(t *testing.T) {
	db, cleanup := qtest.TestDB(t)
	defer cleanup()

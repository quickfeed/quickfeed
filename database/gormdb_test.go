...
var dbImplementations = []struct {
	name   string
	dbFunc func(*testing.T) (database.Database, func())
}{
	{"gormdb", qtest.TestDB},
	{"bundb", qtest.TestBunDB},
}

func runForAll(t *testing.T, f func(*testing.T, database.Database)) {
	t.Helper()
...
func TestDBGetUser(t *testing.T) {
	runForAll(t, func(t *testing.T, db database.Database) {
		if _, err := db.GetUser(10); !errors.Is(err, database.ErrNotFound) {
			t.Errorf("have error '%v' wanted '%v'", err, database.ErrNotFound)
		}
	})
}
...
func TestDBSetAdminNoRecord(t *testing.T) {
	...
	runForAll(t, func(t *testing.T, db database.Database) {
		if err := db.UpdateUser(&qf.User{ID: id, IsAdmin: true}); !errors.Is(err, database.ErrNotFound) {
			t.Errorf("have error '%v' wanted '%v'", err, database.ErrNotFound)
		}
	})
}

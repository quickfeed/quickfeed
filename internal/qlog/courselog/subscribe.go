...
type Subscription struct {
	...
	// report. It is only touched by publish, under the store's mutex.
	dropped int
}
...
func (s *Store) Subscribe(org string) *Subscription {
	sub := &Subscription{
		store: s,
		org:   sanitize(org),
		ch:    make(chan *qf.CourseLogEntry, subscriptionBuffer),
	}
	s.subsMu.Lock()
...
// subscriber that has dropped entries is told so before the next one it
// receives, so a teacher knows the view has a hole in it rather than
// silently missing records. Callers must hold the store's subsMu.

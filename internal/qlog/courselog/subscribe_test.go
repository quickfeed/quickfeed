...
// logger, and must be told that it did.
func TestSubscriptionDropsWhenFull(t *testing.T) {
	...
	for range subscriptionBuffer + 10 {
		select {
		case entry := <-sub.C():
			if entry.GetFields()[droppedLabel] != "" {
				dropped = entry
			}
		default:
		}
	...
	// The gap is reported before the next entry the subscriber receives, so
	// draining the buffer alone does not surface it; one more record does.
	if dropped == nil {
		logger.Info("after the gap")
		for range 2 {
			select {
			case entry := <-sub.C():
				if entry.GetFields()[droppedLabel] != "" {
	...
	if dropped == nil {
		t.Fatal("no dropped-entry report delivered; the teacher would not know the view has a hole")
	}

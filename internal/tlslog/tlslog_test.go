...
const (
	...
	brokenPipe      = prefix + "192.0.2.24:39114: write: broken pipe"
...
func TestClassify(t *testing.T) {
	tests := []struct {
		...
		{name: "broken pipe", msg: brokenPipe, wantReason: reasonDisconnected, wantHandshake: true},
		...
		{
			// The client also chooses its ALPN protocol names, and Go echoes
			// them into the error, so a protocol name imitating an autocert
			// rejection must not be counted as one.
			name:          "protocol name imitating a rejected host",
			msg:           prefix + `192.0.2.25:44444: tls: client requested unsupported application protocols (["h2\" not configured in HostWhitelist"])`,
			wantReason:    reasonProtocolProbe,
			wantHandshake: true,
		},
		{
			name:          "protocol name imitating an autocert error",
			msg:           prefix + `192.0.2.26:44446: tls: client requested unsupported application protocols (["acme/autocert: missing server name"])`,
			wantReason:    reasonProtocolProbe,
			wantHandshake: true,
		},
		{
...
func TestSummary(t *testing.T) {
	...
	recorder.check(t, []record{
		...
		{
			level: slog.LevelInfo,
			msg:   "TLS handshake failures since the previous summary: unauthorized_sni=3 protocol_probe=2 disconnected=1 timeout=1",
		},
...
func TestSummaryCoversOnlyElapsedInterval(t *testing.T) {
	...
	recorder.check(t, []record{
		{level: slog.LevelInfo, msg: eof, reason: reasonDisconnected},
		{level: slog.LevelInfo, msg: "TLS handshake failures since the previous summary: disconnected=2"},
		{level: slog.LevelInfo, msg: eof, reason: reasonDisconnected},
		{level: slog.LevelInfo, msg: "TLS handshake failures since the previous summary: disconnected=1"},
...
// TestConcurrentFailures checks that a sampler is safe to use from several
// goroutines at once. The logger New returns cannot reach the sampler
// concurrently, since log.Logger serializes writes to its io.Writer, so the
// test writes to the sampler directly: that is the contract the sampler owns
// as an io.Writer, independently of how it is wrapped.
//
// The clock never advances, so every failure falls in one interval, and the
// expected reasons are each sampled exactly once however the writes interleave.
func TestConcurrentFailures(t *testing.T) {
	...
	msgs := []string{rejectedHost, noCipherSuite, eof, ioTimeout, unknownFailure}
	before := make(map[string]float64, len(reasons))
	for _, reason := range reasons {
		before[reason] = counted(t, reason)
	}
	...
	for range goroutines {
		...
		go func() {
			...
			for _, msg := range msgs {
				if _, err := sampler.Write([]byte(msg + "\n")); err != nil {
					t.Error(err)
				}
			}
		}()
	...
	for _, reason := range reasons {
		if got := counted(t, reason) - before[reason]; got != goroutines {
			t.Errorf("counted %v %s failures, want %d", got, reason, goroutines)
		}
	}

	logged := make(map[string]int)
	for _, record := range rec.all() {
		logged[record.reason]++
	}
	want := map[string]int{
		reasonUnauthorizedSNI: 1,
		reasonProtocolProbe:   1,
		reasonDisconnected:    1,
		reasonTimeout:         1,
		reasonUnexpected:      goroutines, // never sampled away
	}
	if !maps.Equal(logged, want) {
		t.Errorf("logged %v records by reason, want %v", logged, want)
	}
}
...
// recorder is a slog.Handler that keeps the records it is given. The records
// are guarded because TestConcurrentFailures writes to a sampler from several
// goroutines at once.
...
// all returns a copy of the records logged so far.
...
func (r *recorder) check(t *testing.T, want []record) {
	...
	if len(records) != len(want) {
		t.Fatalf("logged %d records, want %d:\ngot:  %+v\nwant: %+v", len(records), len(want), records, want)
	}

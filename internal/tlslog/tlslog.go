...
// autocertPrefix is the fixed text that x/crypto/acme/autocert puts in front
// of its own errors. Classification anchors on it so that a field the client
// controls cannot imitate an autocert error; see classify.
const autocertPrefix = "acme/autocert: "
...
// reasons is every reason, in the order they appear in a summary.
// reasonUnexpected must stay last, since expectedReasons drops the final
// element to obtain the reasons that are summarized; add new reasons above it.
var reasons = []string{
...
// option configures the error logger returned by New. The options are
// unexported because production installs the logger with New(), and only the
// tests in this package need to vary the logger, the interval or the clock.
...
// summary renders the counts accumulated since the previous summary. The
// caller holds s.mu. The text claims no duration: a summary is emitted by the
// first failure after the interval elapsed, so it covers at least the
// interval, but an arbitrarily longer quiet stretch too. Adjacent summary
// records are timestamped, and the counter carries the rate.
func (s *sampler) summary() string {
	var b strings.Builder
	b.WriteString("TLS handshake failures since the previous summary:")
...
func classify(msg string) (string, bool) {
	...
	switch {
	// Anchored on the autocert prefix, and checked first: the rejected host
	// name is chosen by the client, and so are the ALPN protocol names that Go
	// echoes into the unsupported application protocols error, so an unanchored
	// substring here would let a protocol probe pass as a rejected host name.
	// detail always starts with the prefix of whichever package produced the
	// error, since failureDetail cuts the message after the client address.
	case strings.HasPrefix(detail, autocertPrefix) &&
		containsAny(detail, "not configured in HostWhitelist", "missing server name"):

...
func TestFibonacciWithRun(t *testing.T) {
	for _, ft := range fibonacciTests {
		t.Run(fibTestName(ft.in), func(t *testing.T) {
			sc := score.GMax(fibTestName(ft.in))
			out := fibonacci(ft.in)
			if out != ft.want {
				sc.Dec()
			}
...
func fibTestName(i uint) string {

...
func TestGetNextReviewer(t *testing.T) {
	...
	IDs := []uint64{1, 2, 3, 4}
	teachers := []*types.User{{ID: 1}, {ID: 2}, {ID: 3}, {ID: 4}, {ID: 5}}
	students := []*types.User{{ID: 1}, {ID: 2}, {ID: 3}}
	for _, ID := range IDs {
...
func TestPublishFeedbackComment(t *testing.T) {
	...
	opt := &scm.IssueCommentOptions{
		...
		Repository:   types.StudentRepoName(qfTestUser),
		Body:         body,
		CommentID:    0,
	}

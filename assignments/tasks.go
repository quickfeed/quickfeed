...
func newTask(contents []byte, assignmentOrder uint32, name string) (*types.Task, error) {
...
func tasksFromAssignments(assignments []*types.Assignment) map[uint32]map[string]*types.Task {
...
func mapTasksByID(tasks []*types.Task) map[uint64]*types.Task {
	taskMap := make(map[uint64]*types.Task)
...
func synchronizeTasksWithIssues(ctx context.Context, db database.Database, sc scm.SCM, course *types.Course, assignments []*types.Assignment) error {
	...
	repos, err := db.GetRepositoriesWithIssues(&types.Repository{
...
func createIssues(ctx context.Context, sc scm.SCM, course *types.Course, repo *types.Repository, tasks []*types.Task) ([]*types.Issue, error) {
	createdIssues := []*types.Issue{}
	for _, task := range tasks {
		...
		createdIssues = append(createdIssues, &types.Issue{
...
func updateIssues(ctx context.Context, sc scm.SCM, course *types.Course, repo *types.Repository, tasks []*types.Task) error {

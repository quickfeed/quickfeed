...
export const useCourseLogStream = (courseID: bigint, initialFilters: CourseLogFilters) => {
    ...
    useEffect(() => {
        ...
        const from = (): Timestamp => latest.current
            ? shift(latest.current, 1)
            : timestampFromDate(new Date(Date.now() - query.filters.window))
    ...
    /**
     * loadOlder fetches one more window of entries from before the oldest one
     * loaded, for a teacher who started watching too late. It is a bounded
     * request, so it ends on its own without disturbing the live stream. It
     * rejects if the request fails, leaving the loaded entries as they were,
     * so the caller can say so without tearing down the live view.
     */
    const loadOlder = async () => {
        const oldest = current?.result?.entries[0]?.time
        if (!oldest || loadingOlder.current) {
            return
        }
        loadingOlder.current = true
        try {
            const to = shift(oldest, -1)
            const stream = api.client.courseLogStream({
                courseID: query.courseID,
                from: timestampFromDate(new Date(floorMs(to) - query.filters.window)),
                to,
                repository: query.filters.repository,
                level: query.filters.level,
            })

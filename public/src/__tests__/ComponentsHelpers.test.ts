...
describe("generateRow", () => {
    const tests = [
        ...
        },
        {
            // Group with ID: 2
            // Has no submissions
            desc: "Group{ID: 2} should have rows {-}",
            enrollment: create(GroupSchema, { ID: 2n }),
            generator: (s: Submission) => ({ value: `${s.ID}` }),
            want: [{ value: "", link: "https://github.com//" }, "-"]
        }
    ]

...
export const parseWindow = (text: string): number | null => {
    ...
    // UNITS answers for inherited names too, so a plain lookup makes
    // "1 constructor" a function and amount * unit a NaN that reads as a valid
    // window and reaches new Date as one.
    const key = match[2]
    const unit = Object.hasOwn(UNITS, key) ? UNITS[key] : undefined

...
const CourseLogTable = ({ entries, rows, controls, onLoadOlder }: CourseLogTableProps) => {
    ...
    // choices. With no rows there is no table to draw, but Load older has to
    // stay within reach: a window that came back empty is exactly when a
    // teacher wants to look further back, and so is a search that matched
    // nothing on screen.
    if (rows.length === 0) {
        return onLoadOlder && (
            <div className="flex justify-center shrink-0">
                <button type="button" className="btn btn-sm" onClick={onLoadOlder}>Load older</button>
            </div>
        )
    }

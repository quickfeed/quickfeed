...
const EditCourse = () => {
    ...
    const courses = state.courses.map(c => {
        ...
        const CancelOrEdit = selected ? "Cancel" : "Edit"
        const toggleCourseForm = () => { selected ? setCourse(undefined) : setCourse(c) }
        data.push(
            c.name, c.code, c.tag,
            c.year.toString(), c.slipDays.toString(),

            <button className={`clickable ${badge}`} onClick={toggleCourseForm}>
                {CancelOrEdit}
            </button>
        )
        return data

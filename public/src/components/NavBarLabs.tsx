...
const NavBarLabs = () => {
    ...
    const labs = (): JSX.Element[] => { 
        ...
        let links = state.submissions[state.activeCourse]?.map((submission, index) => {
            ...
            return (
                <li style={{position: "relative"}} key={submission.getAssignmentid()}>
                    <div id="icon">
                    ...
                    <div style={{position: "absolute", borderBottom: "1px solid green", bottom: 0, left: 0, right: `${percentage}%`, borderColor: `${ submission.getScore() - rand * 100 >= assignment.getScorelimit() ? "green" : "yellow"}`  }}></div>
                </li>
            )
        })
        return links

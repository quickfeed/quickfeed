...
export const Groups = (props: RouteComponentProps<{id?: string | undefined}>) => {
    ...
    const makeGroups = state.groups[courseID]?.map(group => {
        return (
            <ul className="list-group list-group-flush">
                ...
                { group.getEnrollmentsList().map(enrol => 
                    <li className="list-group-item"><img src={enrol.getUser()?.getAvatarurl()} style={{width: "40px", borderRadius: "50%", marginRight: "10px"}}></img>{enrol.getUser()?.getName()} <span>{enrol.getSlipdaysremaining()}</span></li>
                )}
            </ul>
        )
    })

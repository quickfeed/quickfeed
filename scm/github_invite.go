...
func (s *GithubSCM) AcceptInvitations(ctx context.Context, opt *InvitationOptions) (string, error) {
	...
	if err != nil {
		// Return the original refresh token in case of an error.
		return opt.RefreshToken, fmt.Errorf("failed to exchange refresh token for %s: %w", opt.Login, err)
	}
	...
	for _, repo := range []string{qf.InfoRepo, qf.AssignmentsRepo, qf.StudentRepoName(opt.Login)} {
		...
		if err != nil {
			return refreshToken, fmt.Errorf("failed to fetch invitations for repository %s: %w", repo, err)
		}
		...
		for _, invite := range repoInvites {
			...
			if _, err := userSCM.Users.AcceptInvitation(ctx, invite.GetID()); err != nil {
				return refreshToken, fmt.Errorf("failed to accept invitation for repository %s: %w", invite.Repo.GetName(), err)
			}
		}
	}
	...
	if _, _, err := userSCM.Organizations.EditOrgMembership(ctx, "", opt.Owner, &github.Membership{State: &state}); err != nil {
		return refreshToken, fmt.Errorf("failed to accept invitation to organization %s: %w", opt.Owner, err)
	}

...
- **qcm**: the course manager for teachers;
  - `clone` fetches a course's `tests` and `assignments` repositories,
  - `run` runs one assignment's tests, and
  - `check` verifies that the course's test environment works.
  - Run `go run ./cmd/qcm -help` for details.
  - See [Checking the Test Environment Locally](teacher.md#checking-the-test-environment-locally)
